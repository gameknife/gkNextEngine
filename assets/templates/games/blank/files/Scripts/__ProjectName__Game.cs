...
public sealed class {{ProjectName}}Game : NextGameInstance
{
    ...
    // Tuning, read from Content/configs/tuning.json. The defaults are what the game does when the
    // file is missing or leaves a value out, so a half-written config still runs.
    private float spinSpeed = 1.2f;
    private Vector3 cubeColor = new(0.95f, 0.55f, 0.16f);
    ...
    /// <summary>
    /// Reads this project's own config. <c>GameContent</c> resolves a path inside the project's
    /// <c>Content/</c> folder, so the game never names where it was installed; press Rebuild C# after
    /// editing the file and the running host sees the new values on the next Play.
    /// </summary>
    private void LoadTuning()
    {
        byte[] data = GameContent.ReadFile("configs/tuning.json");
        if (data.Length == 0)
        {
            Log.Warn("[{{ProjectName}}] Content/configs/tuning.json not found; using defaults");
            return;
        }

        // JsonDocument rather than JsonSerializer: the serializer is reflection-based and would need
        // a source-generated context to survive NativeAOT.
        using JsonDocument document = JsonDocument.Parse(data);
        JsonElement root = document.RootElement;
        if (root.TryGetProperty("spinSpeed", out JsonElement speed) && speed.ValueKind == JsonValueKind.Number)
        {
            spinSpeed = speed.GetSingle();
        }
        if (root.TryGetProperty("cubeColor", out JsonElement color) && color.ValueKind == JsonValueKind.Array &&
            color.GetArrayLength() == 3)
        {
            cubeColor = new Vector3(color[0].GetSingle(), color[1].GetSingle(), color[2].GetSingle());
        }
    }
    ...
    protected override void BeforeSceneRebuild()
    {
        ...
        cubeNode = SceneBuild.AddRenderNode("{{ProjectName}}_Cube",
            new RenderNodeSpec(boxModel, SceneBuild.AddLambertianMaterial(cubeColor))

...
/// <summary>
/// AOT-safe manual loader for the same JSON data consumed by the C++ application: the project's
/// Content/configs, which both read.
/// </summary>
public sealed class BrotatoDatabase
{
    // Relative to projects/Brotato3D/Content.
    ...
    {
        byte[] data = GameContent.ReadFile(Root + filename);
        if (data.Length == 0)
        {
            throw new FileNotFoundException($"Brotato3DCSharp could not read {GameContent.Path(Root + filename)}");
        }

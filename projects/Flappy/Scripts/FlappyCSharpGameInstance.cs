...
{
    // The project's own sounds, under projects/Flappy/Content/sounds. Resolved once, not per play:
    // GameContent.Path builds a new string every call.
    private readonly string flapSfx = GameContent.Path("sounds/flap.wav");
    private readonly string scoreSfx = GameContent.Path("sounds/score.wav");
    private readonly string hitSfx = GameContent.Path("sounds/hit.wav");

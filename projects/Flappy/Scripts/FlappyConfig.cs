...
/// <summary>
/// Loads the same JSON files FlappyCpp reads — the project's Content/configs — through the
/// engine's package filesystem.
/// </summary>
/// <remarks>
...
{
    // Relative to projects/Flappy/Content.
    private const string GameplayPath = "configs/gameplay.json";
    private const string ReplayPath = "configs/replay.json";
    ...
    public static GameplayConfig LoadGameplay()
    {
        ...
        byte[] contents = GameContent.ReadFile(GameplayPath);
    ...
    public static ReplayConfig LoadReplay()
    {
        ...
        byte[] contents = GameContent.ReadFile(ReplayPath);
        if (contents.Length == 0)
        {
